...
Иконки для web, Android, iOS, macOS и Windows **уже сгенерированы** и лежат в платформенных папках. Исходник: `assets/icon/app_icon.png` (красная **A** на прозрачном фоне, по центру).

Для **web** favicon используется прозрачный PNG. Для **iOS/Android/desktop** лаунчер-иконки собираются с чёрным фоном `#000000` (требование iOS и adaptive icon на Android).

Пересобрать иконки после замены `assets/icon/app_icon.png`:
