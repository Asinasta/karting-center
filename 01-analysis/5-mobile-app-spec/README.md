# ТЗ на приложение «Апекс»

> Этап 5. Детальное техническое задание на клиентское приложение картинг-центра.

## Технология

Приложение реализуется на **Flutter + Dart**.

Технические решения:

- REST API по OpenAPI из [../api/](../api/).
- Публичный просмотр списка/карточек слотов без токена; JWT access/refresh tokens для бронирования и персональных разделов.
- Защищённое хранение токенов через системное защищённое хранилище и согласованный Flutter-плагин.
- Системный push с регистрацией токена в `registerPushToken`.
...
См. [09_Логики/_INDEX.md](09_Логики/_INDEX.md): OTP, доступность мест, цена, отмена, фильтры, карта, push, состояния экранов, лояльность, оценка маршала.
